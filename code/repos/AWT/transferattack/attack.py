...
class Attack(object):
    ...
    """
    def __init__(self, attack, model_name, epsilon, targeted, random_start, norm, loss,
                 device=None, **kwargs):
        """
        ...
        if "awt" in kwargs.keys():
            self.awt = kwargs["awt"]
            self.sam_lr = kwargs["lr"]
            self.sam_rho = kwargs["rho"]
    ...
    def get_param_grad(self, loss, parameter, **kwargs):

        return torch.autograd(loss, parameter, retain_graph=False, create_graph=False)[0]
